  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] flex items-center justify-center p-6 selection:bg-brand-500/30">
      <div className="w-full max-w-md bg-[#0d0d10] border border-white/5 p-10 rounded-[40px] shadow-2xl relative overflow-hidden">
        {/* Modern Glass Glow */}
        <div className="absolute -top-24 -left-24 w-48 h-48 bg-brand-500/10 blur-[100px] rounded-full" />
        
        <div className="flex flex-col items-center mb-10 text-center relative z-10">
          <div className="p-3 bg-brand-500 rounded-2xl mb-4 shadow-lg shadow-brand-500/20">
            <Zap className="w-6 h-6 text-white fill-white" />
          </div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Join SwiftDocs</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
          ...
          <button 
            disabled={loading}
            className="w-full bg-brand-500 text-white py-4 rounded-2xl font-bold text-lg hover:brightness-110 transition-all flex justify-center items-center gap-2"
          >
        ...
        <p className="text-center text-gray-500 text-sm mt-8 relative z-10">
          Already have an account? <Link href="/login" className="text-brand-500 font-bold hover:underline">Login</Link>
        </p>
      </div>
    </div>
  )
